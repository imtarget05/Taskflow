...
export function GoogleSignIn({ className }: GoogleSignInProps) {
  ...
  async function handleClick() {
    // Luôn thử redirect — server sẽ trả 503 + redirect với google_error nếu thực sự chưa cấu hình.
    // Tránh chặn nhầm khi fetch status fail do mạng/proxy.
  ...
  // Trong khi đang kiểm tra, vẫn hiện nút bấm được để không chặn luồng chính
  if (status === null) {
    return (
      <Button type="button" variant="secondary" className="w-full flex items-center justify-center gap-2" onClick={handleClick}>
